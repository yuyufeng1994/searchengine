...
public class ChineseSpellTool {
    ...
    public static String findsm(String py, StringBuffer result) {
    ...
    public static String findym(String py, int index, StringBuffer result) {
        int temp = 0;
        ...
        if (temp != 0) {
            result.append(String.copyValueOf(py2, 0, temp) + " ");
            py = py.substring(temp);
        }
        return py;
    }
    ...
    public static String trimSpellWithChinese(String str) {
        // abc  len = 3
        // 012
        String endStr = "";
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (isChinese(c + "")) {
                ...
                for (int j = i; j < str.length(); j++) {
                    if (j == str.length() - 1) {
                        String pyStr = str.substring(i, j + 1);
                        resultStr.append(" " + trimSpell(pyStr) + "");
                        i = j;
                        break;
                    }
                    char cj = str.charAt(j);
                    if (isChinese(cj + "")) {
                        String pyStr = str.substring(i, j);
                        resultStr.append(" " + trimSpell(pyStr) + "");
                        i = j - 1;
                        break;
                    }
        ...
        return resultStr.toString() + " " + endStr;
    }

    public static void main(String[] args) {
        //long start = System.currentTimeMillis();
        String spell = "hangzhou";
        String result = ChineseSpellTool.trimSpellWithChinese(spell);
        System.out.println(result);
        result = ChineseSpellTool.trimSpellWithChinese(spell);
